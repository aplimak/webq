...
function submitRound(content: Element) {
    ...
    try {
        ...
        showErrorToast(
            `Error while saving round results: ${e instanceof Error ? e.message : e}`,
        );
        return;
    }
    ...
    function submitConfig() {
        ...
        if (maxScoreInput && maxScoreInput instanceof HTMLInputElement) {
            try {
                ...
                showErrorToast(
                    `Error while updating max score: ${e instanceof Error ? e.message : e}`,
                );
                return;
            }
        }
        ...
        try {
            ...
            showErrorToast(
                `Error while saving players data: ${e instanceof Error ? e.message : e}`,
            );
            return;
        }
    ...
    dialog.body.querySelector("#add-player")?.addEventListener("click", () => {
        function submitNewPlayer() {
            ...
            if (!playerId) {
                ...
                const exists =
                    playerDefs.filter((item) => item.id === playerId).length !== 0;
                if (exists) {
            ...
            const playerName = (
                innerDialog.body.querySelector("#player-name") as HTMLInputElement
            ).value;
            if (!playerName) {
